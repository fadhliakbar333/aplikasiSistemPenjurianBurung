...
class EditSesiScreen extends ConsumerStatefulWidget {
  final SesiModel sesi;
  const EditSesiScreen({super.key, required this.sesi});

  @override
  ConsumerState<EditSesiScreen> createState() => _EditSesiScreenState();
}

class _EditSesiScreenState extends ConsumerState<EditSesiScreen> {
  ...
  late Map<String, int> _kategoriBobot;
  int _totalBobot = 0;

  @override
  void initState() {
    super.initState();
    _namaController = TextEditingController(text: widget.sesi.nama);
    _kuotaController = TextEditingController(text: widget.sesi.kuota.toString());
    _hargaController = TextEditingController(text: widget.sesi.hargaTiket.toString());
    _deskripsiController = TextEditingController(text: widget.sesi.deskripsi);
    _kategoriBobot = Map<String, int>.from(widget.sesi.kategoriBobot);
            ...
            children: [
              DropdownButtonFormField<KategoriPenilaian>(
                ...
                items: KategoriPenilaian.values
                    .where((k) => !_kategoriBobot.containsKey(k.name)) // Hanya tampilkan yg belum dipilih
  ...
  Future<void> _updateSesi() async {
    if (_formKey.currentState!.validate()) {
      ...
      final updatedSesi = SesiModel(
        ...
        juriIds: widget.sesi.juriIds,
        eventNama: widget.sesi.eventNama,
        eventLokasi: widget.sesi.eventLokasi,
        eventTanggal: widget.sesi.eventTanggal,
      );

      try {
        await ref.read(firestoreServiceProvider).updateSesi(updatedSesi);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Sesi berhasil diperbarui!')),
          );
